...
class SigMaNetConv(MessagePassing):
  ...
  # Possiamo utilizzare  questa funzione per elaborare la parte Tx0=I
    ...
    def split_vector(self, vector):
        ...
        first_half = vector[:midpoint]
        second_half = vector[midpoint:]
        return first_half, second_half

    def split_tensor(self, vector):
        midpoint = len(vector[0]) // 2
        first_half = vector[:, :midpoint]
        second_half = vector[:, midpoint:]
        return first_half, second_half
    ...
    def forward(
        ...
        norm_imag_1, norm_imag_2 = self.split_vector(self.norm_imag) 
        norm_real_1, norm_real_2 = self.split_vector(self.norm_real)
        edge_index_1, edge_index_2 = self.split_tensor(self.edge_index)
        ...
        Tx_1_real_real_1 = self.propagate(edge_index_1, x=x_real, norm=norm_real_1, size=None).to(torch.float) # x_real - norm_real
        Tx_1_real_real_2 = self.propagate(edge_index_2, x=x_real, norm=norm_real_2, size=None).to(torch.float) # x_real - norm_real
                    #print("Tx_1_real_real", Tx_1_real_real)
        out_real_real = out_real_real + torch.matmul((Tx_1_real_real_1 + Tx_1_real_real_2), self.weight[1])
                   #print("output_real_real", out_real_real)
        Tx_1_imag_imag_1 = self.propagate(edge_index_1, x=x_imag, norm=norm_imag_1, size=None).to(torch.float) # x_imag - norm_imag
        Tx_1_imag_imag_2 = self.propagate(edge_index_2, x=x_imag, norm=norm_imag_2, size=None).to(torch.float) # x_imag - norm_imag
                    #print("Tx_1_imag_imag", Tx_1_imag_imag)
        out_imag_imag = out_imag_imag + torch.matmul((Tx_1_imag_imag_1 + Tx_1_imag_imag_2), self.weight[1])
                    #print("output_imag_imag", out_imag_imag)
        Tx_1_imag_real_1 = self.propagate(edge_index_1, x=x_imag, norm=norm_real_1, size=None).to(torch.float) # x_imag - norm_real
        Tx_1_imag_real_2 = self.propagate(edge_index_2, x=x_imag, norm=norm_real_2, size=None).to(torch.float) # x_imag - norm_real
        out_imag_real = out_imag_real + torch.matmul((Tx_1_imag_real_1 + Tx_1_imag_real_2), self.weight[1])
        #
        Tx_1_real_imag_1 = self.propagate(edge_index_1, x=x_real, norm=norm_imag_1, size=None).to(torch.float) # x_real - norm_imag
        Tx_1_real_imag_2 = self.propagate(edge_index_2, x=x_real, norm=norm_imag_2, size=None).to(torch.float) # x_real - norm_imag
        out_real_imag = out_real_imag + torch.matmul((Tx_1_real_imag_1 + Tx_1_real_imag_2), self.weight[1])
        #
...
class Future_node_classification_new(nn.Module):
    def __init__(self, num_features:int, hidden:int=2, K:int=1, label_dim:int=2, \
        ...
        super(Future_node_classification_new, self).__init__()
